...
export function getConditions(bundle: any): ParsedCondition[] {
...
export function getMedications(bundle: any): ParsedMedication[] {
    if (!bundle?.entry) return [];
...
export function getAllergies(bundle: any): ParsedAllergy[] {
    if (!bundle?.entry) return [];
...
export function getEncounters(bundle: any): ParsedEncounter[] {
    if (!bundle?.entry) return [];

    const encounters = bundle.entry
        .filter((e: any) => e.resource?.resourceType === 'Encounter')
        .map((e: any) => {
            const r = e.resource;
            return {
...
export function getObservations(bundle: any): ParsedObservation[] {
    if (!bundle?.entry) return [];
...
export function getImmunizations(bundle: any): ParsedImmunization[] {
    if (!bundle?.entry) return [];
...
export function getProcedures(bundle: any): ParsedProcedure[] {
    if (!bundle?.entry) return [];
...
export function getCarePlans(bundle: any): ParsedCarePlan[] {
    if (!bundle?.entry) return [];

    return bundle.entry
        .filter((e: any) => e.resource?.resourceType === 'CarePlan')
        .map((e: any) => {
            const r = e.resource;
            return {
