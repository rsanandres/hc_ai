...
from api.agent.tools.schemas import CalculationResponse
...
def calculate_gfr(
    ...
    if age <= 0 or creatinine <= 0:
        return CalculationResponse(
            success=False,
            error="age and creatinine must be positive.",
            result=None,
        ).model_dump()
...
def calculate_bmi(weight_kg: float, height_cm: float) -> Dict[str, Any]:
    ...
    if weight_kg <= 0 or height_cm <= 0:
        return CalculationResponse(
            success=False,
            error="weight_kg and height_cm must be positive.",
            result=None,
        ).model_dump()
...
def calculate_creatinine_clearance(
    ...
    if age <= 0 or weight_kg <= 0 or creatinine <= 0:
        return CalculationResponse(
            success=False,
            error="age, weight_kg, and creatinine must be positive.",
            result=None,
        ).model_dump()
