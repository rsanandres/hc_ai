...
from api.agent.tools.schemas import FDAResponse
...
async def get_drug_shortages(drug_name: str, limit: int = 5) -> Dict[str, Any]:
    ...
    Searches using openFDA harmonized fields (generic_name, brand_name) and
    product name fields for comprehensive results.
    """
    if not drug_name.strip():
        return FDAResponse(
            success=False,
    ...
    # Escape quotes in drug name for query safety
    escaped_name = drug_name.replace('"', '\\"')
    
    # Try multiple search strategies for better coverage:
    # 1. openFDA harmonized fields (works across all FDA endpoints)
    # 2. Direct product name field (if available in shortages endpoint)
    # Using OR to combine strategies
    query = (
    ...
    if "error" in data:
        ...
        # 404 Not Found means no shortages exist for this drug - treat as success with empty results
    ...
    shortages = []
    for item in data.get("results", []):
        # Extract more comprehensive information from the shortage record
        shortage_info = {
            "status": item.get("status", ""),
        ...
        # Add product name if available (from openfda or direct field)
        ...
        # Also check for direct product_name field
